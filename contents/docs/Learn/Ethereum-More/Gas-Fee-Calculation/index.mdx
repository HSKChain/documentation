...
## HSKChain's Fee Model

[HSKChain](/docs/Build-on-HashKey-Chain/Fee) employs a dual-fee structure that builds upon Ethereum's model while optimizing for its Layer 2 architecture.

### Dual-Fee Structure

Each transaction on HSKChain includes two distinct fee components:
...
**L2 Execution Fee:**
- Covers computational resources on HSKChain
...
1. **Data Compression**: HSKChain employs advanced data compression techniques to minimize L1 security fees
...
For detailed information on HSKChain's fee structure, optimization techniques, and current fee rates, refer to the [HSKChain Transaction Fee documentation](/docs/Build-on-HashKey-Chain/Fee).
